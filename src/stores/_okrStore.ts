...
export const useOKRStore = create<OKRState>((set, get) => ({
 ...
 fetchCycles: async () => {
  try {
    ...
    console.log('[DEBUG CONTEXT] Contexto para usuário atual:', context);
    ...
    console.log('[DEBUG] Ciclos carregados depois:', data);

    if (data && data.length > 0) {
      const cyclesWithFormattedDates = data.map((cycle) => ({
        ...cycle,
        start_date_text: cycle.start_date,
        end_date_text: cycle.end_date,
      }));

      set({ cycles: cyclesWithFormattedDates, selectedCycleId: cyclesWithFormattedDates[0].id });
    } else {
...
createCycle: async (cycleData) => {
  const { canCreateCycle } = usePermissions(); // usando hook aqui

  if (!canCreateCycle) {
    toast.error('Você não tem permissão para criar ciclos.');
    return;
  }
...
updateCycle: async (cycleId, updates) => {
  const { canEditCycle } = usePermissions();

  if (!canEditCycle) {
    toast.error('Você não tem permissão para editar ciclos.');
    return;
  }
...
deleteCycle: async (cycleId) => {
  const { canDeleteCycle } = usePermissions();

  if (!canDeleteCycle) {
    toast.error('Você não tem permissão para excluir ciclos.');
    return;
  }
  ...
  try {
    ...
    console.error('[Erro real ao excluir ciclo]', error);
    set({ error: (error as Error).message });
    throw error;
  }
},
